...
async function submitForm() {
  ...
  const payload = {
    ...
    holy:         Array.from(document.getElementById("holy").selectedOptions).map(o => o.value).filter(v => v).join("\n"),
    sport:        Array.from(document.getElementById("sport").selectedOptions).map(o => o.value).filter(v => v).join("\n"),
  };
